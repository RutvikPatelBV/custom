...
class Employee(models.Model):
    ...
    emp_role = fields.Selection([('frontend_developer', 'Frontend Developer'),('backend_developer', 'Backend Developer'),('designer', 'Designer'),('qa_tester', 'QA Tester')],string="Role", required=True)
    emp_gender=fields.Selection([('male','Male'),('female','Female'),('others','Others')],string="Gender", required=True)
    joining_date = fields.Date(string='Joining Date', required=True)
    emp_experience = fields.Integer(string='Experience', compute='_compute_experience', store=True , help="This is automaticly calculate based on joining date")
    ...
    def _compute_experience(self):
        for emp in self:
            if emp.joining_date:
                ...
                emp_experience = today.year - join_date.year - ((today.month, today.day) < (join_date.month, join_date.day))
